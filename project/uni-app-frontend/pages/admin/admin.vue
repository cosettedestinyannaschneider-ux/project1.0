<template>
  <view class="admin-container">
    ...
    <view v-if="activeTab === 'config'" class="panel">
      <view class="panel-header">
        <text class="panel-title">AI 大模型配置 ({{ configList.length }})</text>
        <button class="mini-btn add-btn" @click="openAddConfigModal">添加模型</button>
      </view>
      <view v-if="configList.length === 0" class="empty-hint">
        <text>暂无模型配置，点击"添加模型"创建第一个配置</text>
      </view>
      <view class="config-list">
        <view v-for="cfg in configList" :key="cfg.id" class="config-card card">
          <view class="config-main">
            <view class="config-name-row">
              <text class="config-name">{{ cfg.name }}</text>
              <text v-if="cfg.is_active" class="active-badge">当前使用</text>
            </view>
            <text class="config-detail">模型: {{ cfg.model_name }}</text>
            <text class="config-detail">地址: {{ cfg.base_url }}</text>
            <text class="config-detail">最大Token: {{ cfg.max_tokens }} | 温度: {{ cfg.temperature }} | 超时: {{ cfg.timeout_ms }}ms</text>
          </view>
          <view class="config-actions">
            <button v-if="!cfg.is_active" class="mini-btn activate-btn" @click="activateConfig(cfg)">启用</button>
            <button class="mini-btn edit-btn" @click="openEditConfigModal(cfg)">编辑</button>
            <button class="mini-btn delete-btn" @click="deleteConfig(cfg)">删除</button>
          </view>
        </view>
      </view>
    </view>
    ...
    <!-- 模型配置编辑弹窗 -->
    <view v-if="showConfigModal" class="modal-mask" @click="closeConfigModal">
      <view class="modal-content" @click.stop="">
        <text class="modal-title">{{ editingConfigId ? '编辑模型配置' : '添加模型配置' }}</text>
        <scroll-view class="modal-body config-modal-body" scroll-y>
          <input class="modal-input" v-model="configForm.name" placeholder="配置名称（如：DeepSeek 生产环境）" />
          <input class="modal-input" v-model="configForm.base_url" placeholder="API 地址（如：https://ark.cn-beijing.volces.com/api/v3）" />
          <view class="password-input-wrapper">
            <input
              class="modal-input"
              v-model="configForm.api_key"
              type="text"
              :password="!showConfigApiKey"
              :placeholder="editingConfigId ? 'API Key（留空不修改）' : 'API Key'"
            />
            <image
              class="toggle-password-icon"
              :src="showConfigApiKey ? '/static/icons/eye-off.png' : '/static/icons/eye.png'"
              @tap="showConfigApiKey = !showConfigApiKey"
            />
          </view>
          <input class="modal-input" v-model="configForm.model_name" placeholder="模型 ID（如：ep-20260326164423-7hbm6）" />
          <input class="modal-input" v-model.number="configForm.max_tokens" type="number" placeholder="最大 Token（默认 4096）" />
          <input class="modal-input" v-model.number="configForm.temperature" type="digit" placeholder="温度 0-2（默认 0.7）" />
          <input class="modal-input" v-model.number="configForm.timeout_ms" type="number" placeholder="超时毫秒（默认 60000）" />
        </scroll-view>
        <view class="modal-btns">
          <button class="modal-btn cancel" @click="closeConfigModal">取消</button>
...
const showConfigApiKey = ref(false)
...
const fetchAiConfigs = () => {
  uni.request({
    url: apiUrl('/api/admin/config/ai/list'),
    method: 'POST',
    header: getAdminHeaders(),
    data: { admin_id: user.value.id },
    success: (res) => {
      if (res.data.success) configList.value = res.data.data || []
    }
  })
}
...
const openEditConfigModal = (cfg) => {
  ...
  configForm.value = {
    ...
    timeout_ms: cfg.timeout_ms ?? 60000
  }
  showConfigApiKey.value = false
  showConfigModal.value = true
}
...
const saveConfig = () => {
  ...
  if (!form.name || !form.base_url || !form.model_name) {
    return uni.showToast({ title: '请填写配置名称、API地址和模型ID', icon: 'none' })
  }
  if (!editingConfigId.value && !form.api_key) {
    return uni.showToast({ title: '请填写 API Key', icon: 'none' })
  }

  const isEdit = !!editingConfigId.value
  const url = isEdit ? '/api/admin/config/ai/update' : '/api/admin/config/ai/add'
  ...
  uni.request({
    ...
    success: (res) => {
      if (res.data.success) {
        uni.showToast({ title: isEdit ? '配置已更新' : '配置已添加' })
        closeConfigModal()
        fetchAiConfigs()
      } else {
...
const activateConfig = (cfg) => {
  uni.showModal({
    title: '切换模型',
    content: `确定将「${cfg.name}」设为当前使用的模型吗？`,
    success: (res) => {
      if (!res.confirm) return
      uni.request({
        url: apiUrl('/api/admin/config/ai/activate'),
        method: 'POST',
        header: getAdminHeaders(),
        data: { admin_id: user.value.id, id: cfg.id },
        success: (res) => {
          if (res.data.success) {
            uni.showToast({ title: '已切换为当前模型' })
            fetchAiConfigs()
          } else {
            uni.showToast({ title: res.data.message || '操作失败', icon: 'none' })
          }
...
const deleteConfig = (cfg) => {
  uni.showModal({
    title: '确认删除',
    content: `确定删除模型配置「${cfg.name}」吗？`,
    success: (res) => {
      if (!res.confirm) return
      uni.request({
        url: apiUrl('/api/admin/config/ai/delete'),
        method: 'POST',
        header: getAdminHeaders(),
        data: { admin_id: user.value.id, id: cfg.id },
        success: (res) => {
